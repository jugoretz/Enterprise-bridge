/*
 This does a few things.  and would probably be easier on a board with more pins available.
 The wav files are stored on an SD card and played directly from there.
 1. We play a wav file (opening.wav) once on startup
 2. We play a different file (bridge_activate.wav) when that one is done
 3. We play a third file (bridge_looper.wav) and continue looping that one indefinitely
 4. We wait for input from a button.  At any time when any of those three above are playing, if
    the button is pressed (quick press) we stop playing what was playing, and instead we play a different
    file (red_alert.wav), looping it exactly three times. while it's playing, we flash a red led
    (or lots of them using a transistor). We also light an indicator LED with a steady light.  
    When the three times are done, we go back to looping bridge_looper.wav.  
    This is repeated whenever that button is pressed. 
 5. (secret mute function) A long press on that button will mute all sounds until it gets another
    long press.  unfortunately, I wasn't able to make it so that the short press doesn't trigger
  while the long press is underway.  oh well.
*/
